...
async def on_message(message):
    ...
    if message.content.lower() == "!menu":
        menu_message = await message.channel.send(
            ...
            "5️⃣ Reset to Default Settings\n"
            "6️⃣ Disable/Enable Embed (Thumbnail Only)\n"
            "❌ Close Menu"
        )
        ...
        try:
            ...
            if str(reaction.emoji) == "1️⃣":
                ...
                channel_settings[message.channel.id] = default_settings.copy()
                await menu_message.edit(content="✅ **Settings Reset to Default!**")

            elif str(reaction.emoji) == "6️⃣":
                settings['embed_enabled'] = not settings['embed_enabled']
                status = "enabled" if settings['embed_enabled'] else "disabled"
                await menu_message.edit(content=f"✅ **Embed has been {status}!**")
